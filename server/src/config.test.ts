 ...
 * comment on it in config.ts), so it keeps its own literal default in sync by
 * convention. This test is the actual enforcement: it fails loudly the moment the two
 * numbers drift, which is exactly the bug that shipped three times in this project's
 * first three commits, each of which touched both files.
 */
